...
describe('star_schema_version', () => {
  it('applies once and stays at target on a second boot', async () => {
    ...
    await db.waitReady;
    const readSql = (name: 'init.sql' | 'init-d1.sql' | 'init-d1b.sql' | 'init-d1c.sql' | 'init-d1d.sql' | 'init-d1-triggers.sql') =>
  ...
  it('keeps every drizzle table name in public/*.sql or the version DDL', () => {
    ...
    const sql = [
      'init.sql', 'init-d1.sql', 'init-d1b.sql', 'init-d1c.sql', 'init-d1d.sql',
