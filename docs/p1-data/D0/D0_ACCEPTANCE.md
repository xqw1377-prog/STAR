# P1-DATA-D0 · 验收矩阵（D0 ACCEPTANCE）· rev5
...
P1-DATA-D0 rev4 = SUPERSEDED（20c2bca；rev4 终审 CHANGES-REQUIRED，十项落表矛盾）
...
| 18 | 外部审计 C1–C6 处置已登记（J0/S0/D1-B 映射） | TEMPORAL_RESEARCH_CONTEXT §6 → rev5 移至 AUDIT_DISPOSITION.md | 签署后按序 |
| 19 | rev4 十项落表矛盾闭合 | FACT §3.1/§3.2/五态/仅 payload_ref/部分唯一索引/§5.5 关系表/§7 artifact 注册表；HEALTH 五态六率+0-vs-null；TEMPORAL 唯一键含 source+转移图 | T31–T35（A/B） |
...
| 阶段 | 范围 | 测试 |
|---|---|---|
| **D1-A 事实底座** | A0/A1/R/N schema、不可变边界、幂等、并发、处置状态机 | T01, T02, T03a, T05, T06, T06b, T14, T20, T21, T23, T24, T25, T31, T32, T33 |
| **D1-B 时态事实管道** | fixture→raw→parser→fact 血缘、冲突、双回放、Narrative/Lifecycle 点时事实、写入前时态校验、固定时钟 | T03b, T04, T15, T16, T26, T29, T30, T34, T35 |
| **D1-C 合成语料** | 150 案例、oracle 独立、无前视、家族分组 | T07, T07a, T08, T09, T10, T17, T18 |
| **D1-D 数据健康** | 投影、六率五态、优先级、归因、零样本与 0-vs-null | T11, T12, T13, T19, T22, T27, T28 |

新增测试（rev5）：**T31**（回执+结局事件同事务原子落库、DEFERRABLE FK、崩溃无中间态）、
**T32**（部分唯一索引：fact_local_key NULL 多行被拒、非 NULL 同键被拒）、
**T33**（receipt_relation 多重关系：一回执同时 CONTESTS + SUPERSEDES 可表达）、
**T34**（Lifecycle 转移图/连续性/同刻冲突 → lifecycle=UNKNOWN）、
**T35**（artifact 注册表取回 + content_hash 验证，HISTORICAL 重演完整）。
